...
from settings import MOPAC_COMMAND, ORCA_COMMAND, GAUSSIAN_COMMAND, PROCS, MEM
...
def get_ase_calc(calculator, method):
    '''
...
def orca_opt(coords, atomnos, constrained_indexes=None, method='PM3', title='temp', read_output=True):
    '''
...
def gaussian_opt(coords, atomnos, constrained_indexes=None, method='PM6', title='temp', read_output=True):
    '''
...
def optimize(calculator, TS_structure, TS_atomnos, mols_graphs, constrained_indexes=None, method='PM7 GEO-OK', max_newbonds=2, title='temp', debug=False):
    ...
    if calculator == 'MOPAC':
        ...
        opt_coords, energy, success = gaussian_opt(TS_structure, TS_atomnos, constrained_indexes, method=method, title=title)
...
def ase_adjust_spacings(docker, structure, atomnos, constrained_indexes, mols_graphs, method='PM7', max_newbonds=0, traj=None):
    ...
    atoms.calc = get_ase_calc(docker.options.calculator, method)
